...
/**
Returns the target vector of the current camera
*/
glm::vec3 GetCurrentTarget(void)
{
	switch (_cam_type) {
	case CAMERA_MANIPULATOR:
		return cam_manipulator.getTargetVec();
		break;
	case TRACKBALL: // constant distance
...
void cursorCallback(GLFWwindow *window, double x, double y) {
	//cout << "g_change_texture_blend = " << g_change_texture_blend << endl;
...
{
	cam_manipulator.camera_key_Callback(window, key, scancode, action, mods); // added for navigation
	//trackball.keyCallback(window, key, scancode, action, mods);
...
GLFWwindow* initWindow(void)
{
	...
	glfwSetKeyCallback(window, key_callback); 	//// modified navigation based on keyboard input
