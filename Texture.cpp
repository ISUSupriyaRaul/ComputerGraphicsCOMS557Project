...
#ifdef WIN32
string  GLTexture::_glsl_name[1] = { "treeTexture"}; //"tex", "texture_blend"

string  GLMultiTexture::_glsl_names[4] = { "terrainTex", "blendmap", "mud","walkway"};
#endif
...
int GLTexture::loadAndCreateTexture(string path_and_file)
{
	...
	cout << "loadAndCreateTexture started running \n" << endl;
	...
	cout << "loadAndCreateTexture channels = "<< channels << " width= "<< width << " height= "<< height <<" \n" << endl;
...
/*!
 ...
 @param path_and_file_texture_3 - path and file of the third image.
 @param path_and_file_texture_4 - path and file of the fourth image.
 @return int - the texture id when the texture was sucessfully loaded.
 */

int GLMultiTexture::loadAndCreateTextures(string path_and_file_texture_1, string path_and_file_texture_2, string path_and_file_texture_3, string path_and_file_texture_4)
{
	...
	if (data1 == NULL || data2 == NULL || data3 == NULL || data4 == NULL)return -1;

	//**********************************************************************************************
	// Texture generation - terrain grass
	...
	GLint whichID0;
	glGetIntegerv(GL_TEXTURE_BINDING_2D, &whichID0);
	cout << "The id of the texture bound to the currently active texture unit, whichID0 = " << whichID0 << endl;
	...
	GLint whichID1;
	glGetIntegerv(GL_TEXTURE_BINDING_2D, &whichID1);
	cout << "The id of the texture bound to the currently active texture unit, whichID1 = " << whichID1 << endl;
	...
	GLint whichID3;
	glGetIntegerv(GL_TEXTURE_BINDING_2D, &whichID3);
	cout << "The id of the texture bound to the currently active texture unit, whichID3 = " << whichID3 << endl;
...
bool GLMultiTexture::addVariablesToProgram(GLuint program, int variable_index)
{
	...
	cout << "The number of texture units available, texture_units = " << texture_units  <<endl;
	...
	//GLint whichTexUnit;
	//glGetIntegerv(GL_ACTIVE_TEXTURE, &whichTexUnit);
	//cout << "The active texture unit, whichTexUnit = " << whichTexUnit << endl;
	//GLint whichID;
	//glGetIntegerv(GL_TEXTURE_BINDING_2D, &whichID);
	//cout << "The id of the texture bound to the currently active texture unit, whichID = " << whichID << endl;
	...
	*/
	...
	//GLint whichID2;
	//glGetIntegerv(GL_TEXTURE_BINDING_2D, &whichID2);
	//cout << "The id of the texture bound to the currently active texture unit, whichID2 = " << whichID2 << endl;
	/*
	glActiveTexture tells OpenGL which texture unit we want to use. GL_TEXTURE0 is the first texture unit, so we will just use that.
	*/
	glActiveTexture(GL_TEXTURE3); //GL_TEXTURE3
	...
	*/
	...
	//glBindTexture(GL_TEXTURE_2D, 0); ////maybe nothing helpful
...
int GLMipMapTexture::loadAndCreateTexture(string path_and_file)
{
	...
	// This generates the midmaps
	//glGenerateMipmap(GL_TEXTURE_2D); // what is the use of it here??


	//glBindTexture(GL_TEXTURE_2D, 0); // does not make the river go Black
