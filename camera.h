...
public:
	...
	/**
	Navigation with keyboard inputs, Set eyepos and forward direction
	*/
	virtual void camera_key_Callback(GLFWwindow * window, int key, int scancode, int action, int mods) = 0;
};
...
public:
    ...
    */
	...
	Navigation with keyboard inputs, Set eyepos and forward direction
	*/
	virtual void camera_key_Callback(GLFWwindow * window, int key, int scancode, int action, int mods);

	/*!
	Gets the target vector to adjust the camera view
	*/
	virtual glm::vec3 getTargetVec(void);
