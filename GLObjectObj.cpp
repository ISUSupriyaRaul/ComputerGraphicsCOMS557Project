	...
	_file_and_path(filename)
{
	...
	_file_ok = false;
	_file_ok = load_obj(filename.c_str(), _vertices, _normals, _elements, _textures);
...
bool GLObjectObj::load_obj(const char* filename, vector<glm::vec3> &vertices, vector<glm::vec3> &normals, vector<GLuint> &elements, vector<glm::vec2> &textures)
{
	...
	while (getline(in, line))
	{
		...
		{
			istringstream s(line.substr(3));
			glm::vec2 n; s >> n.x; s >> n.y; // OpenGL starts from the bottom left of the corner while Blender starts from the top left
...
void GLObjectObj::initVBO(void)
{
	...
	// textures
	int locTex = glGetAttribLocation(_program, "in_Texcoords");
	...
	glVertexAttribPointer((GLuint)locTex, 2, GL_FLOAT, GL_FALSE, 0, 0); // Set up our vertex attributes pointer
...
void GLObjectObj::initShader(void)
{
	...
	glBindAttribLocation(_program, 2, "in_Texcoords");
