...
void CameraManipulator::init_camera_param(void)
{
	...
	// aaded for navigation
	_keyCode = -1;
	_eyePos = glm::vec3(0.0f, 0.0f, 2.0f);
	_forwardDir = glm::vec3(0.0f, 0.0f, -1.0f);
	_upwardDir = glm::vec3(0.0f, 1.0f, 0.0f);
...
{
	if (_leftMouseButtonEvent != 0 || _rightMouseButtonEvent != 0)
...
/*!
Gets the target vector to adjust the camera view
*/
//virtual
glm::vec3 CameraManipulator::getTargetVec(void)
{
	if (_leftMouseButtonEvent != 0 || _rightMouseButtonEvent != 0)
		return glm::vec3(0.0, 0.0, 0.0);

	if (_keyCode != 0) {
		return (_eyePos+_forwardDir);
	}
}

void CameraManipulator::camera_key_Callback(GLFWwindow * window, int key, int scancode, int action, int mods)
{
	...
	case GLFW_PRESS: // 1
		...
		if (_keyCode == GLFW_KEY_UP) {
			_eyePos += _forwardDir*2.0f;
		}
		else if (_keyCode == GLFW_KEY_DOWN) {
			_eyePos -= _forwardDir*2.0f;
			//std::cout << "Down Key pressed." << std::endl;
		}
		else if (_keyCode == GLFW_KEY_LEFT) {
			glm::quat q = glm::angleAxis(0.02f, glm::vec3(0, 1, 0)); 
			_forwardDir = q* _forwardDir;
			_rotationMatrix += glm::toMat4(q);
			//std::cout << "Left key pressed." << std::endl;
		}
		else if (_keyCode == GLFW_KEY_RIGHT) {
			glm::quat q = glm::angleAxis(-0.02f, glm::vec3(0, 1, 0));
			_forwardDir = q* _forwardDir;
			_rotationMatrix += glm::toMat4(q);
			//std::cout << "Right key pressed." << std::endl;
		}
		else if (_keyCode == GLFW_KEY_X) {
			glm::quat q = glm::angleAxis(0.02f, glm::vec3(1, 0, 0));
			_forwardDir = q* _forwardDir;
			_rotationMatrix += glm::toMat4(q);
		}
		else if (_keyCode == GLFW_KEY_U) {
			_eyePos += _upwardDir*2.0f;
			//std::cout << "U key pressed." << std::endl;
		}
		else if (_keyCode == GLFW_KEY_D) {
			_eyePos -= _upwardDir*2.0f;
	...
	case GLFW_REPEAT: // 2
		...
		if (_keyCode == GLFW_KEY_UP) {
			...
			glm::quat q = glm::angleAxis(0.01f, glm::vec3(0, 1, 0));
			_forwardDir = q* _forwardDir;
			_rotationMatrix += glm::toMat4(q);
		}
		else if (_keyCode == GLFW_KEY_RIGHT) {
			...
			glm::quat q = glm::angleAxis(0.01f, glm::vec3(1, 0, 0));
			_forwardDir = q* _forwardDir;
			_rotationMatrix += glm::toMat4(q);
		}
		else if (_keyCode == GLFW_KEY_U) {
